...
아직은 준비된 페이지가 아니지만 썸네일 관리를 할 때 각 항목들의 페이지가 있어야 하기 때문에 그 페이지의 링크에 사용할 예정입니다. 아무튼 나머지는 다음 글에서 고민을 해보도록 하고 이번 글은 여기서 마쳐야겠습니다.

# 마치며
Supabase를 통해서 썸네일 이미지를 업로드하고, 설정 데이터들도 데이터베이스에 저장을 해서 관리하기 편하게 만드는 발판을 마련했습니다. 다음 글은 지금까지 무시하고 있었던 디자인을 잡고, 모양을 좀 개선하는 시간을 가져야겠습니다.
